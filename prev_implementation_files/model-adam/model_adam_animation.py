'''
model_adam_animation.py

A version of Carolyn Sun's "model.py" slightly tweaked by Adam in some places 
to output 2D color plots of membrane displacement that can be easily packaged 
into an animated GIF with ImageMagick. Most print statements were removed as 
well for any potential speed gain (even though it's probably not much).

Most changes were just related to plot formatting. I don't think any changes 
were made to how the calculations are actually performed.

Some suggestions, just for consideration: Future iterations could save output 
to a subdirectory, and perhaps have class definitions and methods packaged in 
a module to be imported by individual scripts for various test cases. This 
would make individual scripts a little more readable. (I know functionality, 
results, and other things have been taking priority thus far, and that's 
fine!)

Lastly: For final SURF report purposes in LaTeX, when generating "still" 
figures, you may want to consider saving as PDF or some other vector graphics 
format. SVG may also work well for MS PowerPoint.
...
class membrane_response:
    def __init__(self, impulse_times, dt=1e-8, t_max=1e-5, a=0.008, b=0.005, modes=10, h=5e-8, rho=2850, sigma=250e6, eta=0.5):
        ...
        self.p0 = 1e-5  # pressure amplitude, can adjust later if wanting to specify strength of force
    ...
    def calculate_response(self):
        ...
        for m in range(1, self.num_modes + 1):
            for n in range(1, self.num_modes + 1):
                ...
                # Adam's change: Made a much smaller rectangle in the center 
                # to better see the initial outward propagation
                pS_mn = self.p_smn(m, n, ((self.a / 2) - (self.a / 20)), ((self.a / 2) + (self.a / 20)), ((self.b / 2) - (self.a / 20)), ((self.b / 2) + (self.a / 20)), self.a, self.b)
                #print("(m, n): (", m, ", ", n, ") || pS_mn: ", pS_mn)
    ...
    def plot_displacement(self, w_total, t):
        ...
        for ti in range(0, len(t), int(len(t)/100)): # adjust later, have the factor that len(t) is divided by depend on time steps
            ...
            plt.imshow(w_total[ti], cmap='magma', interpolation='nearest', extent=[0,self.a,0,self.b], vmin=-cbar_max_val, vmax=cbar_max_val)
            plt.colorbar(label = 'Displacement')
            plt.title(f'Membrane Displacement Response at Time = {t[ti]:.2e} s') # Changed format specification to scientific notation
            plt.xlabel('x')
            plt.ylabel('y')
            plt.savefig(f'displacement_{ti:03d}.png', format="png") # Changed format specification to have leading zeroes so ImageMagick plays nice with it
            plt.show()
            plt.close(fig) # Close figure after done plotting
...
impulse_times = [0] # Adam's change: Just to see the wave behavior from one pulse clearly
